...
public class OpenLineageIntegration {
    ...
    public void emitKafkaToKafkaEvent(
        ...
        OpenLineage.OutputDataset outputDataset = (OpenLineage.OutputDataset) buildKafkaDataset(outputTopic, KafkaDatasetType.TOP_SALES);
    ...
    private OpenLineage.Dataset buildKafkaDataset(String topic, KafkaDatasetType type) {
        ...
        return switch (type) {
            case SALES_ENRICHED -> ol.newInputDatasetBuilder()
                    ...
                    .build();
            case TOP_SALES -> ol.newOutputDatasetBuilder()
                    .namespace(jobNamespace)
                    .name(topic)
                    .facets(facetsBuilder.build())
                    .build();
    ...
    private java.util.List<OpenLineage.SchemaDatasetFacetFields> getSchemaFields(KafkaDatasetType type) {
        return switch (type) {
            ...
            case TOP_SALES -> java.util.List.of(
                    ol.newSchemaDatasetFacetFields("city_name", "STRING", null, 1L, null),
                    ol.newSchemaDatasetFacetFields("sale_date", "DATE", null, 2L, null),
                    ol.newSchemaDatasetFacetFields("total_amount", "DECIMAL", null, 3L, null),
                    ol.newSchemaDatasetFacetFields("total_units", "INT", null, 4L, null)
            );
        };
    }
